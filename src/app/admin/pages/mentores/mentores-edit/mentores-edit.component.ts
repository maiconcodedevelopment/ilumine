...
import { TIPO_GENERO, TIPO_ESCOLARIDADE, MentorProfile } from 'src/app/models/user.model';
import { MentoresService } from 'src/app/services/mentores.service';

@Component({
  selector: 'app-mentores-edit',
  templateUrl: './mentores-edit.component.html'
})
export class MentoresEditComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.mentoresService.getById(id).subscribe((result: MentorProfile) => {
      ...
      this.angForm.controls.dataNascimento.setValue(this.convertDate(result.dataNascimento));
      ...
      this.angForm.controls.biografia.setValue(result.biografia);
      this.angForm.controls.numeroMaximoMentorias.setValue(result.numeroMaximoMentorias);
    });
  }
  ...
  Salvar(): void {
    if (this.angForm.valid) {
      ...
      this.mentoresService.editUser(this.angForm.value).subscribe(result => {
        this.showloading = !this.showloading;
        if(result == null) {
